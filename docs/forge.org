...
instructions that cover not just the happy path but also many of the
...
sections to Magit buffers.  If you would like to prevent that, you
have to set ~forge-add-default-bindings~ and ~forge-add-default-sections~
to ~nil~ before ~magit~ (not just ~forge~) is loaded.
...
Forges equivalent of ~git clone~ is ~forge-add-repository~, which has to
