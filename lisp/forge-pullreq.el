...
(defun forge-pullreq-at-point ()
  (or (magit-section-when pullreq)
      (when-let ((post (magit-section-when post)))
        (if (forge-pullreq-p post)
            post
          (forge-get-pullreq post)))))

(defvar magit-pullreqs-section-map
...
(defun forge-insert-pullreqs ()
  (when-let* ((repo (forge-get-repository nil))
    ...
    (magit-insert-section section (pullreqs pullreqs t)
      ...
      (if (oref section hidden)
          (oset section washer
                (apply-partially 'forge--insert-pullreqs section repo))
        (forge--insert-pullreqs section repo)))))

(defun forge--insert-pullreqs (section repo)
  (let* ((pullreqs (oref section value))
